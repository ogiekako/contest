...
public class SuffixArray{
	...
	public int[] suffixArray(String s) {
		int n = s.length();
		for(int i=0;i<n-1;i++)if(s.charAt(i) <= s.charAt(n-1))
	...
	private int[] suffixArray(int[] a,final int numLetter){
		...
		for(int i=n-1,j=numLMS;i>=1;i--)if(i==n-1 || S_Type[i] && !S_Type[i-1]) {// i : LMS
			i2LMS[i]=--j;
			LMS2i[j]=i;
			SA[--tail[a[i]]] = i;
		}
		// step2
		for(int i=0;i<n;i++)if(SA[i]>0 && !S_Type[SA[i]-1]) {
			SA[head[a[SA[i]-1]]++] = SA[i]-1;
		}
		System.arraycopy(to,0,tail,0,numLetter);
		// step3
		for(int i=n-1;i>=0;i--)if(SA[i]>0 && S_Type[SA[i]-1]) {
			SA[--tail[a[SA[i]-1]]] = SA[i]-1;
		}
		// construct a1
		int[] a1=new int[numLMS];
		int numLet1=-1;
		for(int i=0,bef=-1;i<n;i++)if(SA[i]>0 &&  S_Type[SA[i]] && !S_Type[SA[i]-1]) {// SA1[i] : LMS
			boolean same=true;
			if(bef==-1)same=false;
			else {
	...
	public int[] calcLCP(int[] SA,String s) {
		...
		int[] lcp=new int[n-1];
		for(int i=0,h=0;i<n;i++)if(rank[i]<n-1) {
