...
*/
public class LinkCutTreeNode implements Comparable<LinkCutTreeNode>, DynamicTreeNode {
    public int compareTo(LinkCutTreeNode o) {
        return id - o.id;
    }
    @Override
    ...
    LinkCutTreeNode upper; // left
    LinkCutTreeNode lower; // right
    /*
    パスの親を表すのにも使う.
    ...
    public LinkCutTreeNode() {}

    public LinkCutTreeNode(int id) {
        this.id = id;
    }
...
//        debug("reverse", this);
        LinkCutTreeNode tmp = upper; upper = lower; lower = tmp;
...
//        debug("getParent", id);
        expose();
//        debug("after expose ; ");
//        for (LinkCutTreeNode node : nodes) {
...
//        if(cur.rev)throw null;
    ...
    private void splay() {
        ...
        if (rev) throw null;
        while (!isPathRoot()) {
            LinkCutTreeNode p = parent, g = p.parent;
 ...
 x   y         y   z
     ...
     */
    void expose() {
        for (LinkCutTreeNode r = null, p = this; p != null; r = p, p = p.parent) {
     ...
     */
    public void link(DynamicTreeNode parent) {
        LinkCutTreeNode _parent = (LinkCutTreeNode) parent;
    ...
    public LinkCutTreeNode root() {
        expose();// already reversed by expose.
        LinkCutTreeNode res = this;
...
////////////////////////////
    ...
    // verified, O(log n)
    public LinkCutTreeNode lowestCommonAncestor(LinkCutTreeNode other) {
        LinkCutTreeNode a = this;
        if (a == other) return a;
        LinkCutTreeNode ra = a.root();
        LinkCutTreeNode rb = other.root();
        if (ra != rb) return null;
        LinkCutTreeNode pr = a.pathRoot();
        if (pr != rb) {
    ...
    private LinkCutTreeNode pathRoot() {
        splay();
        LinkCutTreeNode res = this;
        while (res.upper != null) res = res.upper;
        res.splay();
    ...
    public boolean isConnectedTo(LinkCutTreeNode other) {
        return root() == other.root();
    }

    public LinkCutTreeNode secondRoot() {
        expose();// already reversed by expose.
