...
public class LinkCutTreeTest {
    ...
    public void testRoot() throws Exception {
        Random rnd = new Random(124234098L);
        for (int i = 0; i < 100; i++) {
            int n = 100;
            long seed = rnd.nextLong();
            int c = root(seed, n, 10000);
            Assert.assertEquals(10000, c);
        }
    }

    private int root(long seed, int n, int m) {
        DynamicTree[] nodes = new DynamicTree[n];
        for (int i = 0; i < n; i++) nodes[i] = new LinkCutTree();
        return DynamicTreeTest.test_Link_Cut_Root(seed, n, m, nodes);
    }
    ...
    public void testLowestCommonAncestorTime() {
        ...
        LinkCutTree[] nodes = new LinkCutTree[n];
        for (int i = 0; i < n; i++) nodes[i] = new LinkCutTree();
...
//            System.err.println(Arrays.toString(ps)+" "+q[i]+" "+x[i]+" "+y[i]);
            if (q[i] == 0) {
                ...
                LinkCutTree res = nodes[x[i]].lowestCommonAncestor(nodes[y[i]]);
                if (p == -1) {
...
//                    System.err.println(v);
    ...
    public void testEvert_getParent_secondRoot() {
        ...
        class SolverStupid {
            ...
            public void check(LinkCutTree[] nodes) {
                ...
                for (int i = 0; i < n; i++) {
                    LinkCutTree t = nodes[i].getParent();
        ...
        LinkCutTree[] nodes = new LinkCutTree[n];
        for (int i = 0; i < n; i++) nodes[i] = new LinkCutTree(i);
        for (int i = 0; i < m; i++) {
            solver.check(nodes);
            int q = rnd.nextInt(5);
            if (q == 0) {// add
                ...
                for (int j = 0; j < 100; j++) {
                    int a = rnd.nextInt(n);
                    LinkCutTree t = nodes[a].getParent();
                    if (t == null) continue;
                    int b = t.id;
                    nodes[a].cut();
                    int c = solver.cut(a);
                ...
                LinkCutTree lca = nodes[a].lowestCommonAncestor(nodes[b]);
                ...
                int a = rnd.nextInt(n);
                LinkCutTree secondRoot = nodes[a].secondRoot();
