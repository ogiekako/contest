...
public class DenseGraphUtils {
    ...
    private static boolean isConnectedDfs(boolean[][] graph, int x, int y, boolean[] visited) {
        ...
        for (int i = 0; i < graph.length; i++)
            if (graph[x][i] && !visited[i] && isConnectedDfs(graph, i, y, visited)) return true;
        return false;
    }
    ...
    public static boolean isEdgeConnectedGraph(boolean[][] undirectedGraph) {
        ...
        boolean[] visited = new boolean[n];
        for (int i = 0; i < n; i++)
            if (ArrayUtils.contains(undirectedGraph[i], true)) {
                cnt++;
                if (que.isEmpty()) {
                    que.offer(i);
                    visited[i] = true;
        ...
        while (!que.isEmpty()) {
            ...
            cnt--;
            for (int j = 0; j < n; j++)
                if (undirectedGraph[i][j] && !visited[j]) {
                    visited[j] = true;
                    que.offer(j);
                }
        }
    ...
    private static boolean bipartiseDfs(int ptr, boolean isLeft, boolean[] isLeftSide, boolean[] visited, boolean[][] graph) {
        ...
        for (int i = 0; i < graph[ptr].length; i++)
            if (graph[ptr][i]) {
    ...
    private static int vertexCoverUpTo0(boolean[][] graph, boolean[] used, int[] cost, int k) {
        ...
        int i, j = 0;
        loop:
        for (i = 0; i < n; i++) if (!used[i]) for (j = 0; j < i; j++) if (!used[j] && graph[i][j]) break loop;
        if (i == n) return 0;
