...
public class MathUtilsTest {
    ...
    public void testCombination() throws Exception {
        ...
        for (int i = 0; i < CMod.length; i++) for (int j = 0; j < CMod[0].length; j++){
    ...
    public void testFactorize(){
        ...
        for(int from=1;from<10;from++)for(int to=from;to<10;to++){
        ...
        for(long from= (long) 1e12;from<=1e12+10;from++)for(long to= from;to<1e12+10;to++){
            PrimeDecomposition[] fs = MathUtils.factorize(from, to);
            if(fs.length!=to-from)throw new AssertionError();
            for (int i = 0; i < fs.length; i++){
                long v = from + i;
