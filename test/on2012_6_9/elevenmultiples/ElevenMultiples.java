package on2012_6_9.elevenmultiples;
   ...
   public int countMultiples(String[] pieces) {
    ...
    private long[] even(long[][][] wayE, int cntEven, int cntOdd) {
       ...
       int minus = (cntOdd + 1) / 2;
        ...
        for(int i=0;i<=cntEven;i++) for (int k = 0; k < 11; k++){
            int j = cntEven - i;
    ...
    private long[][][] calcDp(int[] even, int cntEven) {
        ...
        way[0][0][0] = 1;
        for (int i = 0; i < cntEven + 1; i++) for (int j = 0; j < cntEven + 1; j++)if(i+j<cntEven) for (int k = 0; k < 11; k++){
