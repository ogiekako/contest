package on2012_6_7.thequestionsandanswersdivone;
...
public class TheQuestionsAndAnswersDivOne {
   public int find(int questions, String[] answers) {
       ...
       dp[0][0] = 1;
       for (int i = 0; i < questions; i++) for (int j = 0; j < 1 << n; j++){
           for (int k = 1; k < 1 << n; k++)if((j&k)==0 && ((mask&k)==k||(mask&k)==0))dp[i+1][j|k] += dp[i][j];
       }
