package on2012_5_26.chromaticnumber;
...
public class ChromaticNumber {
    public int minColors(String[] _graph) {
        ...
        for (int k = 0; k < n; k++) for (int i = 0; i < n; i++) for (int j = 0; j < n; j++)graph2[i][j] |= graph2[i][k] && graph2[k][j];
        boolean[] vs=new boolean[n];
        int res = 0;
        for (int i = 0; i < n; i++)if(!vs[i]){
            int c=0;
            int A=-1,B=-1,C=-1;
            for (int j = 0; j < n; j++)if(graph2[i][j]){
                c++;vs[j]=true;
                if(A==-1)A=j;
                else if(B==-1)B=j;
                else if(C==-1)C=j;
