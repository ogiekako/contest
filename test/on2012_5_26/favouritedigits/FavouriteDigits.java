package on2012_5_26.favouritedigits;
   ...
   public long findNext(long N, int digit1, int count1, int digit2, int count2) {
       ...
       for (int i = 0;; i++){
           ...
           for (int j = 0; j < 20; j++) for (int k = 0; k < 20; k++) for (int l = 0; l < 2; l++)
               for (int m = 0; m < 2; m++)if(dp[i][j][k][l][m] < Long.MAX_VALUE){
                   if(m==0 && l==1 && j>=count1 && k>=count2 && dp[i][j][k][l][m] >= N)res = Math.min(res,dp[i][j][k][l][m]);
                   for (int d = 0; d < 10; d++){
