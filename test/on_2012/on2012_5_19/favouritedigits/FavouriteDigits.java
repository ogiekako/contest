...
public class FavouriteDigits {
    public long findNext(long N, int digit1, int count1, int digit2, int count2) {
        ...
        loop:
        for (D = dig; ; D++) {
            for (int c1 = count1; c1 < 22; c1++)
                for (int c2 = count2; c2 < 22; c2++)
                    if (dp[D][c1][c2][1][0]) {
        ...
        for (int d = D; d > 0; d--) {
            ...
            int V = 9;
            for (int c1 = Math.max(0, count1); c1 < 22; c1++)
                for (int c2 = Math.max(0, count2); c2 < 22; c2++)
                    for (int k = 0; k < 2; k++)
                        if (k == 1 || more) for (int z = 0; z < 2; z++)
                            if ((z == 0 || d < D) && dp[d][c1][c2][k][z]) {
