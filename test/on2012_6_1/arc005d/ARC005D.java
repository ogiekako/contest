...
public class ARC005D {
    ...
    private void solve(int c) {
        ...
        need[0] = 0;
        for (int i = 0; i < c; i++){
            for(int j=99;j>=0;j--)if(need[j] < Integer.MAX_VALUE){
                for (int k = 1; k < 10; k++)if(can[k]){
                    int nj = j+k;
        ...
        need2[0] = new int[]{};
        for (int i = 1; i < 100; i++) if(need[i] < Integer.MAX_VALUE){
            for (int k = 1;; k++)if(can[k] && need[i-k]+1 == need[i]){
...
//        System.err.println("B" +Arrays.deepToString(need2));
        ...
        for (int i = 0; i < 19; i++) for (int j = 0; j < 10; j++) for (int k = 0; k <= c; k++)if(dp[i][j][k] < Integer.MAX_VALUE){
...
//                    System.err.println("C " + dp[i+1][nj][nk]+" "+nScore);
        ...
        as = null;
        for (int i = 0; i <= c; i++)if(dp[19][0][i] < Integer.MAX_VALUE){
            if(dp[19][0][i] < res){
                res = dp[19][0][i];
                as = dp2[19][0][i];
            }
